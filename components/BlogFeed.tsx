...
export default function BlogFeed({ userId }: BlogFeedProps) {
  ...
  const [likingBlogs, setLikingBlogs] = useState<Record<string, boolean>>({});
  ...
  const handleLike = useCallback(async (blogId: string) => {
    if (likingBlogs[blogId]) return;

    const blog = blogs.find(b => b._id === blogId);
    if (!blog) return;

    const getLikesId = (id: string | { toString?: () => string }): string => {
      return typeof id === 'string' ? id : id.toString?.() || '';
    };

    const isCurrentlyLiked = blog.likes.some((id) => getLikesId(id) === userId);

    setLikingBlogs(prev => ({ ...prev, [blogId]: true }));

    const optimisticLikes = isCurrentlyLiked
      ? blog.likes.filter(id => getLikesId(id) !== userId)
      : [...blog.likes, userId];

    setBlogs(prev => prev.map(b => 
      b._id === blogId ? { ...b, likes: optimisticLikes } : b
    ));
    ...
    try {
      ...
      if (res.ok) {
        ...
        setBlogs(prev => prev.map(b => 
          b._id === blogId ? { ...b, likes: blog.likes } : b
        ));
      }
    } catch {
      setBlogs(prev => prev.map(b => 
      ...
      setLikingBlogs(prev => ({ ...prev, [blogId]: false }));
    }
  }, [blogs, userId, likingBlogs]);
                    ...
                    <div className="flex gap-6 py-4 text-sm border-t border-zinc-100 dark:border-zinc-800 mt-2">
                      <button
                        ...
                        className={`icon-button flex items-center gap-2 px-3 py-1.5 rounded-xl transition-all hover:bg-zinc-100 dark:hover:bg-zinc-800 ${isLiked
                          ...
                          } ${likingBlogs[blog._id] ? "opacity-50 cursor-not-allowed" : ""}`}
                      >
